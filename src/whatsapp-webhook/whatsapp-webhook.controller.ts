...
export class WhatsappWebhookController {
  ...
  @ApiQuery({
    name: 'hub.mode',
    required: false,
    description: 'Hub mode (subscribe)',
  })
  @ApiQuery({
    name: 'hub.challenge',
    required: false,
  ...
  @ApiResponse({
    status: 403,
    description: 'Verification token mismatch - access denied',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Receive and process incoming messages and events from WhatsApp Business API',
  })
  @ApiBody({
    schema: {
      example: { entry: [{ changes: [{ value: { messages: [] } }] }] },
    },
  })
  @ApiResponse({
    status: 200,
