...
import { WhatsappUtils } from '../common/utilities/whatsapp.utils';
...
export class WhatsappWebhookService {
      ...
      case 'messages':
        ...
        if (message) {
          ...
          switch (message.type) {
            case 'text':
              {
                const userInfo = await this.userService.findByMobile(
  ...
  async onOrderMessageReceived(fromMobileNo: number) {
    ...
    await WhatsappUtils.sendOrderLink(fromMobileNo);
  }
